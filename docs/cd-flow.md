...
- **Scope:** CD only — assume role, resolve coordinates, deploy, soak. No build.
...
| `<app>-infra` | Every AWS resource per env + the GitHub-OIDC deploy role | coordinates published to SSM `/<service>/<env>/…` |
| `<app>-config` | `deploy/<env>.yml` — what version, which knobs; thin caller workflow | a deploy trigger |
| `enterprise-ci-templates` | `cd-template.yml` (logic) + `deploy-defaults.yml` + `preflight-deploy` | — |

`<app>-config` never creates infrastructure; `cd-template.yml` never builds.

## 2. Input documents

| Document | Owned by | Lives in | Supplies |
|----------|----------|----------|----------|
| `deploy/<env>.yml` | App team | `<app>-config` | `service.target`, `release.version`/`strategy`, runtime + scaling knobs. Never resource defs, never secrets. |
| `deploy-defaults.yml` | Platform / SRE | this repo | AWS account + region per env, SSM prefix pattern, OIDC role pattern, default strategy + soak. |
| SSM `/<service>/<env>/…` | `<app>-infra` | AWS | Live resource coordinates (cluster, service, function name, bucket, CodeDeploy app/group…). |
...
1. **Preflight_Deploy** — `preflight-deploy` validates `deploy/<env>.yml` against
   ...
   (`deploy_role`, `ssm_prefix`, `version`, `strategy`, `target`, …).
...
2. **Deploy** — bound to `environment: <env>`, so the GitHub Environment's
   ...
   - read resource coordinates from SSM under `ssm_prefix`,

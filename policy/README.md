...
| `deploy-defaults.yml` | `DeployDefaults` | Shared **non-secret** CD defaults. `common:` = OIDC deploy-role naming pattern. `environments:` = per-env (`dev` / `uat` / `prod`) central `runnerAccount`, default region, default strategy, soak time. Consumed by the `preflight-deploy` action. The app's own deploy-target account and every per-app resource name live in `<app>-config/deploy/<env>.yml` (`infra:`), not here. | No |
